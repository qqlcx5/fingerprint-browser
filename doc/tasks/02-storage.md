...
- [ ] T2 `db/init.ts`：打开 SQLite、建 environments 表（字段按 §5，含索引）；启动时检测库损坏 → 备份为 `.bak` → 重建空库并返回"已重置"标记（由 07 收集进 `app:notices` 队列，kind=db_reset，交界面提示）
- [ ] T3 `db/envDao.ts`：`createEnv / getEnv / listEnvs / updateEnv / deleteEnv`（纯数据库层，不动文件目录）
- [ ] T4 `db/secret.ts`：代理密码 `safeStorage` 加密封装 `encryptSecret / decryptSecret`；不可用时降级机器级混淆并在返回值带 `weak:true` 标记（§8；由 07 收集进 `app:notices` 队列，kind=weak_encryption）；明文仅限主进程内存使用（06/07 启动与代理测试时经 `decryptSecret` 取用，不落日志）
