...
function createTray(): boolean {
  ...
  try {
    ...
    tray = new Tray(image)
    tray.setToolTip('Anta Harness')

    const menu = Menu.buildFromTemplate([
      { label: 'Show Anta Harness', click: () => showWindow() },
      { type: 'separator' },
      { label: 'Quit Anta Harness', click: () => deps?.quit() },
    ])
...
function warnNoTrayOnce(): void {
  ...
  if (Notification.isSupported()) {
    new Notification({
      ...
      body: 'This desktop has no system tray, so Anta Harness will close normally instead of minimizing to the tray.',
...
export function notifyFirstHide(): void {
  ...
  if (Notification.isSupported()) {
    new Notification({
      title: 'Anta Harness is still running',
