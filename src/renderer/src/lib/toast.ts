...
const CODE_TEXT: Record<AppErrorCode, string> = {
  ...
  PROXY_IP_CONFLICT: '该出口 IP 已绑定给其他业务环境',
  PROXY_EGRESS_CHANGED: '代理出口 IP 已变化，请完成重新绑定',
  SECURE_STORAGE_UNAVAILABLE: '系统安全存储不可用，不能保存敏感凭据',
  TOTP_NOT_CONFIGURED: '该环境尚未配置两步验证动态码',
