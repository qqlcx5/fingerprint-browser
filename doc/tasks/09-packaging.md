...
- 生产态 `E2E_SMOKE=1 E2E_RUNTIME=1 pnpm exec electron .` ✓
  - 真实 Chromium 内核准备、环境启动、关闭环境 context 后状态回 idle、重启环境后持久 Cookie 保留、显式 stop 后全部 idle 均 PASS。
- `FP_CHECK_URL=https://browserleaks.com/canvas ... fp-check.cjs` ✓；`FP_CHECK_URL=https://abrahamjuliot.github.io/creepjs/ ... fp-check.cjs` ✓
...
- 未执行：Windows NSIS 安装/卸载；真实不同国家认证代理的并行、认证失败和国家变化确认流。
...
| 8   | Win/mac 安装→运行→卸载，卸载后登录态保留          | △    | mac DMG 包内 E2E PASS；Windows 与完整卸载保留未验证 |
| 9   | 断网首启：内核下载明确失败提示，可重试            | △    | IPC 两次失败重试 PASS；UI 重试按钮未人工验证 |

### 附录 B：性能锚点（§7，随 T5 实测填数）

| 锚点                                 | 达标线        | 实测    |
| ------------------------------------ | ------------- | ------- |
| 单环境启动（内核就绪，不含代理测试） | ≤ 5s          | ☐ 未采样 |
| 10 并行 30 分钟整机内存              | ≤ 6GB，无崩溃 | △ Chromium RSS 求和峰值 8.34GB（非整机物理内存）；约 10 分钟，未满 30 分钟 |
| 单环境 Chromium 内存                 | ≤ 500MB       | ☐ 未单独采样 |
| 主进程空闲内存                       | ≤ 200MB       | ☐ 未采样 |
| 每环境数据目录                       | 100–300MB     | △ 空白 profile 平均 12.5MB，非业务使用量 |
