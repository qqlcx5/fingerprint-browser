...
- [x] T5 增加 SQLite 迁移和 DAO 测试，覆盖旧库升级、出口 IP 重复拒绝、空出口开发环境、导入回滚和敏感字段不导出。当前 E2E 已覆盖旧库升级、V1 导入默认值和敏感字段不回传；出口 IP 冲突/导入回滚真实验证延期到 V2-08。

## 验证记录

- 2026-09-09：`E2E_SMOKE=1 E2E_LEGACY_MIGRATION=1 pnpm exec electron .` 通过，覆盖旧 SQLite 表结构增量迁移、旧 V1 导入默认字段、密码不回传和 TOTP 生命周期。
- 旧版非字符串代理密码记录不会阻断应用启动；可解析的已保存密码会继续保留，无法解析的异常历史数据仅保留代理地址，需用户在该环境中重新保存一次代理。
