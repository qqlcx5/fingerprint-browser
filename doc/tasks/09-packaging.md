...
- [ ] T1 electron-builder 基础配置：appId/产品名/图标/版本、asar、原生模块（better-sqlite3）打包验证 —— **配置已完成**，`pnpm build:unpack` + 包内 `check-native` 验证待阶段 4（并行规则禁跑 build）
...
- [ ] T3 macOS arm64：dmg 验证；签名/公证占位（无证书时记录 TODO）—— **dmg 配置 + TODO 已落**（`identity: null` ad-hoc 签名、`notarize: false`，yml 内注明取证书后的补法），装机验证待阶段 4
